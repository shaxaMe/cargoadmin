<template>
          ...
          >
            ...
            >
              <tr>
                ...
                <th scope="col" class="px-6 py-3">Тип транспорта</th>
                <th scope="col" class="px-6 py-3">Тип кузова</th>
                <th scope="col" class="px-6 py-3">Гос. номер</th>
                <th scope="col" class="px-6 py-3">Грузоподъемность</th>
                <th scope="col" class="px-6 py-3">Объем</th>
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-6 py-4">
                  {{
                    options?.truck_body_type.find((d) => d.id == item.truck_body)[
                      "name"
                    ]
                  }}
                </td>
                <td class="px-6 py-4">
                  {{ item.document.license_plate }}
                </td>
                <td class="px-6 py-4">{{ item.volume }} м³</td>
                <td class="px-6 py-4">{{ item.weight }} т</td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                  <span
                    :class="[
                      ...
                      {
                        'bg-green-100 text-green-800': item.status === 'wait',
                ...
                >
                  <div class="flex justify-end space-x-3">
                    <button class="text-indigo-600 hover:text-indigo-900" @click="getEditData(item.id)">
  ...
  const getDriverAvatar = (driverId, imgUrl) => {
    // TODO: Заменить на реальную логику получения аватара водителя
    return !imgUrl
      ? `https://ui-avatars.com/api/?name=Driver${driverId}&background=random`
      : imgUrl;
  };
  function getEditData(id) {
    useApi(`/v1/driver/vehicle/${id}`).then((res) => {
      if (res) {
